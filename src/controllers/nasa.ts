...
import { cacheOption, NasaEndpoint, NasaSearchParams } from '../interfaces/utils.interface';
...
const fetchNasaDataByEndpoint = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const cacheUtilies = {
        hashKey: String(req.user._id),
        key: cacheKey,
        source: "nasa"
    }
    const nasaData = await fetchNasaData(endpoint as NasaEndpoint,orderedObject, cacheUtilies);
